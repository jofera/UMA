...
public abstract class Game<MOVE> {
	...
	protected int[] maxValueLeaves(GameState state, AlphaBeta ab) {
		// esta función debe devolver dos resultados enteros, los guardamos en un array
		int[] result = new int[2];
		result[0] = Integer.MIN_VALUE; // valor minimax
		result[1] = 0; // contador de hojas visitadas
		if (terminalTest(state)) {
			...
			for (int i = 0; i < successorList.size(); i++) {
				GameState successor = (GameState) successorList.get(i);
				int[] resSuc = minValueLeaves(successor, ab.copy());
				int minimumValueOfSuccessor = resSuc[0]; // valor minimax del suc.
				int hojasSuc = resSuc[1]; // numero de hojas
				result[1] = result[1] + hojasSuc;
				if (minimumValueOfSuccessor > result[0]) {
					result[0] = minimumValueOfSuccessor;
					state.put("next", successor); // esto no es imprescindible
				}
				if (result[0] >= ab.beta()) {
					return result; // PODA: no se examinan más sucesores
				}
				ab.setAlpha(Util.max(ab.alpha(), result[0])); // actualiza alfa
			}
			return result;
		}
	}
	
	protected int[] minValueLeaves(GameState state, AlphaBeta ab) {
		// esta función debe devolver dos resultados enteros, los guardamos en un array
		int[] result = new int[2];
		result[0] = Integer.MAX_VALUE;; // valor minimax
		result[1] = 0; // contador de hojas visitadas
		if (terminalTest(state)) {
			result[0] = computeUtility(state);
			result[1] = 1;
			...
			for (int i = 0; i < successorList.size(); i++) {
				GameState successor = (GameState) successorList.get(i);
				int[] resSuc = maxValueLeaves(successor, ab.copy());
				int maxValueOfSuccessor = resSuc[0]; // valor minimax del suc.
				int hojasSuc = resSuc[1]; // numero de hojas
				result[1] = result[1] + hojasSuc;
				if (maxValueOfSuccessor < result[0]) {
					result[0] = maxValueOfSuccessor;
					state.put("next", successor); // esto no es imprescindible
				}
				if (result[0] <= ab.alpha()) {
					return result; // PODA: no se examinan más sucesores
				}
				ab.setBeta(Util.min(ab.beta(), result[0])); // actualiza beta
			}
			return result;
		}
	}
