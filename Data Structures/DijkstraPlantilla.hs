...
--
-- Alumno: GONZALO FERNANDEZ JAIME
-- Titulación: GRADO EN INGENIERIA DE COMPUTADORES
-------------------------------------------------------------------------------
...
import List (delete, minimumBy) -- consulta delete y minimumBy en Hoogle
...
dijkstraAux g vs rs d = dijkstraAux g (v':vs) (delete v' rs) (D.insert v' (p', cs') d)
   where
      ...
      extensiones = [(d, (p+p,d:cs)) | (o, (p,cs)) <- caminos, (d,p') <- successors g o, d `elem` rs]
      --- seleccionar la extensión de menor coste
      (v', (p', cs')) = minimumBy comparePares extensiones

comparePares (v, (p, cs)) (v', (p', cs')) = compare p p'
...
AVLDictionary('a'->(0,"a"),'b'->(3,"ab"),'c'->(7,"abc"),'d'->(5,"abd"),'e'->(10,"abde"))
