...
public class CommentFacadeREST extends AbstractFacade<Comment> {
    ...
    @GET
    @Path("byPost/{from}/{to}/{postId}")
    @Produces({"application/json"})
    public List<Comment> findRangeByPost(@PathParam("from") Integer from, 
            @PathParam("to") Integer to, 
            @PathParam("postId") Integer postId) {
        Query query = getEntityManager().createNamedQuery("Comment.findByPostId");
        ...
        @GET
        @Path("byPost/count/{postId}")
        @Produces({"text/plain"})
        public String countByPost(@PathParam("postId") Integer postId) {
            Query query = getEntityManager().createNamedQuery("Comment.countByPostId");
            query.setParameter("postId", postId);
